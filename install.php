	...
	if($ispost) {
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if($ins['error']) {
					die('<script type="text/javascript">alert("链接更新服务器失败, 错误为: ' . $ins['error'] . '！");history.back();</script>');
				}
				$archive = $ins['files'];
				if(!$archive) {
...
function tpl_install_db($error = '') {
	...
	<form class="form-horizontal" method="post" role="form">
		...
		<ul class="pager">
			<li class="previous"><a href="javascript:;" onclick="$('#do').val('back');$('form')[0].submit();"><span class="glyphicon glyphicon-chevron-left"></span> 返回</a></li>
			<li class="previous"><a href="javascript:;" onclick="if(check(this)){jQuery('#do').val('continue');if($('input[name=type]').val() == 'remote'){alert('请确认你选择的是离线安装。离线平台请不要主动连接官网，切记！')}$('form')[0].submit();}">继续 <span class="glyphicon glyphicon-chevron-right"></span></a></li>
		</ul>
	</form>
	<script>
		var lock = false;
